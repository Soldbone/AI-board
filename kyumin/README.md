...
- RAG: 글 작성 중 제목 옆 `미리확인` 버튼으로 관련 게시글 3개와 AI 중복/개선 피드백을 생성합니다.
- MCP: 아이디어 상세에서 Video Games MCP Server를 호출해 비슷한 기존 게임을 최대 5개 보여줍니다.
- Agent: 아이디어 상세에서 LLM이 RAG/MCP 도구를 직접 선택해 호출하고, 한 줄 요약, 차별점, 난이도, 개선 제안 3개를 생성합니다.
...
4. 이미 embedding이 저장된 게시글만 PostgreSQL `pgvector` cosine distance로 비교합니다.
5. 관련 게시글 3개를 LLM 입력에 함께 넣어 중복 가능성/개선 제안을 생성합니다.
6. 관련 게시글과 생성 피드백을 함께 반환합니다.

게시글 embedding은 작성자가 OpenAI API Key를 등록한 상태에서 글을 작성하거나 수정할 때 생성합니다. 작성자 Key가 없는 글은 일반 게시글로 저장되지만 RAG 검색 대상에서는 제외됩니다.
...
2. OpenAI 텍스트 모델이 Function Calling으로 `rag_search_posts`, `mcp_search_game_by_title`, `mcp_get_popular_games` 같은 도구 중 다음 행동을 선택합니다.
3. 백엔드가 선택된 RAG/MCP 도구를 실행하고, 결과를 관찰값으로 다시 LLM에 전달합니다.
4. Agent는 최대 3단계 안에서 도구 선택/실행/관찰을 반복하거나 최종 응답을 생성합니다.
5. 최종 아이디어 요약, 차별점, 구현 난이도, 개선 제안 3개는 `ai_analysis_results`에 저장하고 화면에 표시합니다.
