...
- stale 요약 재생성은 전체 댓글 재요약이 아니라 기존 요약과 새 댓글만 사용한다.
- LLM 요약 입력은 MVP에서 최대 700자로 제한한다.

### 직접 했다면 접근법

테스트에서는 summary provider를 mock하지만, 런타임 성공 fallback으로 고정 문자열을 저장하지 않는다. 요약 생성 요청은 `PENDING` row를 저장하고 내부 비동기 작업으로 실제 LLM provider를 호출한다.

### 유의사항

- 요약은 댓글을 대체하지 않는다.
- 삭제된 댓글은 요약 대상에서 제외한다.
- Phase 11 MVP는 루트 댓글 스레드당 최신 요약 1개만 유지한다.
- 이미 최신 SUCCESS 요약이 있으면 새 LLM 호출을 만들지 않는다.
- 상세 구현 하네스는 `phase11_comment_summary_plan.md`를 따른다.
...
- AI가 사실 여부를 최종 판정하는 것처럼 쓰지 않고, 근거 후보를 제시한다고 표현한다.
