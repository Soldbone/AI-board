...
function buildFallbackMapUrl(place: AgentRecommendedPlace) {
  const query = [place.title, place.road_address || place.address].filter(Boolean).join(' ')
            ...
            {agentRecommendation.places.length > 0 && (
              <div className="divide-y divide-slate-200 border-y border-slate-200">
                {agentRecommendation.places.map((place) => {
                  const mapUrl = place.naver_map_url || buildFallbackMapUrl(place)
